...
import { Check, X, Zap, Shield, DollarSign, Users } from 'lucide-react';

// Definir las comparativas permitidas
const COMPARATIVAS = [
  { operador1: 'etb', operador2: 'claro' },
  { operador1: 'etb', operador2: 'movistar' },
  { operador1: 'claro', operador2: 'movistar' },
  { operador1: 'claro', operador2: 'etb' },
  { operador1: 'movistar', operador2: 'etb' },
  { operador1: 'movistar', operador2: 'claro' },
];
...
export async function generateMetadata({ params }: { params: { operador1: string; operador2: string } }): Promise<Metadata> {
  const provider1 = getProviderBySlug(params.operador1);
  const provider2 = getProviderBySlug(params.operador2);
  
  if (!provider1 || !provider2) {
    return {
      title: 'Comparador de Internet en Colombia',
    };
  }
  ...
  return {
    ...
    alternates: {
      canonical: `https://comparadorinternet.co/${params.operador1}-vs-${params.operador2}`,
    },
    openGraph: {
      title: `${nombre1} vs ${nombre2} 2026`,
      description: `Comparación detallada entre ${nombre1} y ${nombre2}`,
      url: `https://comparadorinternet.co/${params.operador1}-vs-${params.operador2}`,
    },
  };
}
...
export default function ComparativaPage({ params }: { params: { operador1: string; operador2: string } }) {
  const provider1 = getProviderBySlug(params.operador1);
  const provider2 = getProviderBySlug(params.operador2);

  if (!provider1 || !provider2) {
    return <div>Comparativa no encontrada</div>;
  }
  ...
  return (
    <>
      ...
      <section className="py-12 bg-gray-50">
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-black mb-8 text-center">Ventajas y Desventajas</h2>
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl font-black mb-8 text-center">¿Cuál es Mejor Para Ti?</h2>

            <div className="grid md:grid-cols-3 gap-6">
              <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-6">
                <Zap className="w-10 h-10 text-blue-600 mb-4" />
                <h3 className="font-bold text-lg mb-3">Para Gaming</h3>
                <p className="text-sm text-gray-700 mb-4">
                  {nombre1 === 'Claro' || nombre2 === 'Claro' 
                    ? 'Claro ofrece mejor estabilidad y cobertura 5G para gaming competitivo.'
                    : 'ETB y Movistar tienen velocidad simétrica ideal para streaming mientras juegas.'}
                </p>
              </div>

              <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-6">
                <Users className="w-10 h-10 text-green-600 mb-4" />
                <h3 className="font-bold text-lg mb-3">Para Teletrabajo</h3>
                <p className="text-sm text-gray-700 mb-4">
                  {nombre1 === 'Movistar' || nombre2 === 'Movistar'
                    ? 'Movistar destaca en velocidad de subida simétrica, crucial para videollamadas.'
                    : 'ETB ofrece excelente soporte técnico y estabilidad para trabajo remoto.'}
                </p>
              </div>

              <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-6">
                <DollarSign className="w-10 h-10 text-purple-600 mb-4" />
                <h3 className="font-bold text-lg mb-3">Mejor Precio</h3>
                <p className="text-sm text-gray-700 mb-4">
                  Ambos operadores tienen ofertas competitivas. Solicita cotización personalizada para tu zona.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
              ...
              <details className="bg-white rounded-lg p-6">
                ...
                <p className="mt-4 text-gray-700">
                  ...
                  de ambos operadores para comparar ofertas específicas en tu zona. Usa nuestro formulario para recibir 
              ...
              <details className="bg-white rounded-lg p-6">
                <summary className="font-bold text-lg cursor-pointer">
                  ¿Puedo cambiar de {nombre1} a {nombre2} sin penalizaciones?
                </summary>
                <p className="mt-4 text-gray-700">
                  Depende de tu contrato actual. Si tienes permanencia mínima vigente, puede haber penalizaciones. 
                  Sin embargo, muchos operadores ofrecen bonificaciones por portabilidad que pueden cubrir estos costos. 
                  Consulta las condiciones de tu contrato actual antes de realizar el cambio.
                </p>
              </details>
            </div>
          </div>
